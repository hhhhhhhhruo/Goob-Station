...
public sealed class ReflectSystem : EntitySystem
{
    ...
    private void OnReflectUserHitscan(Entity<ReflectComponent> ent, ref HitScanReflectAttemptEvent args)
    {
        ...
        if (TryReflectHitscan(ent, args.Target, args.Shooter, args.SourceItem, args.Direction, args.Reflective, args.Damage, out var dir))
        {
    ...
    public bool TryReflectProjectile(Entity<ReflectComponent> reflector, EntityUid user, Entity<ProjectileComponent?> projectile)
    {
        ...
        if (!TryComp<ReflectiveComponent>(projectile, out var reflective) ||
            ...
            !rand.Prob(reflector.Comp.ReflectProb) ||
            !TryComp<PhysicsComponent>(projectile, out var physics))
        {
            return false;
        }

        var rotation = rand.NextAngle(-reflector.Comp.Spread / 2, reflector.Comp.Spread / 2).Opposite();
        ...
        [NotNullWhen(true)] out Vector2? newDirection)
    {
        if ((reflector.Comp.Reflects & hitscanReflectType) == 0x0 ||
            !_toggle.IsActivated(reflector.Owner) ||
            !((reflector.Comp.Reflects & hitscanReflectType) != 0x0 && PredictedRandom(reflector.Owner).Prob(reflector.Comp.ReflectProb)))
        {
            newDirection = null;
            return false;
        }
        ...
        var rand = PredictedRandom(reflector.Owner);
        var spread = rand.NextAngle(-reflector.Comp.Spread / 2, reflector.Comp.Spread / 2);
